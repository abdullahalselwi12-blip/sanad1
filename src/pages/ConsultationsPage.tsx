...
// Observer Pattern
import { consultationSubject } from '@/patterns/observer/ConsultationSubject';
...
export function ConsultationsPage() {
  ...
  // Observer Pattern
  // ==========================================
  const [notificationObserver] = useState(
    () => new NotificationObserver()
  );

  useEffect(() => {
    consultationSubject.subscribe(notificationObserver);

    return () => {
      consultationSubject.unsubscribe(notificationObserver);
    };
  }, [notificationObserver]);

  // ==========================================
  // Load Lawyers + Consultations
  // ==========================================
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      ...
      if (!cancelled) {
        if (lawyerResult.status === 'fulfilled') {
          setLawyers(
            (lawyerResult.value.data || []) as Lawyer[]
          );
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user) {
      toast(
        'يجب تسجيل الدخول لإرسال استشارة',
        'warning'
      );
      return;
    }
    ...
    try {
      // إنشاء الاستشارة
      const createdResult = await createConsultation({
        subject,
        question,
      ...
      // ==========================================
      // Observer Pattern
      // إشعار جميع الـ Observers بإنشاء الاستشارة
      // ==========================================
      if (createdResult?.data?.id) {
        consultationSubject.consultationCreated(
    ...
    <div className="container-page section-padding py-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-navy-500 dark:text-navy-400">
          اطرح سؤالك القانوني واحصل على استشارة من محامٍ معتمد
        </p>
      </div>
            ...
            >
              ...
              {lawyers.map((l) => (
                <option
                  ...
                  value={l.id}
                >
                  {l.profile?.full_name} — {l.specialization}
                </option>
              ))}
            </Select>
        ...
        <div>
          ...
          {!user ? (
            ...
            <EmptyState
              ...
              title="سجل الدخول"
              description="يجب تسجيل الدخول لعرض استشاراتك"
            />
            ...
            <EmptyState
              ...
              description="لم ترسل أي استشارة بعد"
            />
            ...
            <div className="space-y-3">
              ...
              {consultations.map((c) => {
                ...
                const status =
                  CONSULTATION_STATUSES[c.status];
                ...
                return (
                  ...
                  >
                    ...
                    {/* Question */}
                    <p className="text-sm text-navy-600 dark:text-navy-300 mb-2 line-clamp-2">
                      {c.question}
                    </p>
